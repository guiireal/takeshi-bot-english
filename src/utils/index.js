    ...
    try {
      const targetCommand = commands.find((cmd) => {
        if (!cmd?.commands || !Array.isArray(cmd.commands)) {
          errorLog(
            `Error in command of type "${type}": The "commands" property must exist and be an ["array"] with the command names! Erroneous file: ${cmd.name}.js`
          );
      ...
      console.error(`Error processing commands of type "${type}":`, error);
    }
  }
...
exports.getImageBuffer = async (url, options = {}) => {
  try {
    ...
    errorLog(`Error getting image buffer: ${error.message}`);
    throw error;
  }
};
