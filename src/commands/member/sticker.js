/**
 * Developed by: Dev Gui
 * Metadata implementation by: MRX
 *
 * @author Dev Gui
 */
...
module.exports = {
  name: "sticker",
  description:
    "Create stickers from images, gifs or videos (maximum 10 seconds).",
  commands: ["f", "s", "sticker", "fig"],
  usage: `${PREFIX}sticker (tag or reply to an image/gif/video)`,
  handle: async ({
    isImage,
    isVideo,
    downloadImage,
    ...
    if (!isImage && !isVideo) {
      throw new InvalidParameterError(
        `You need to tag or reply to an image/gif/video!`
      );
    }
    ...
    try {
      if (isImage) {
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            console.error(
              `Attempt ${attempt} to download image failed:`,
              downloadError.message
            );

            if (attempt === 3) {
              throw new Error(
                `Failed to download image after 3 attempts: ${downloadError.message}`
              );
            }
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            console.error(
              `Attempt ${attempt} to download video failed:`,
              downloadError.message
            );

            if (attempt === 3) {
              throw new Error(
                `Failed to download video after 3 attempts. Connection problem with WhatsApp.`
              );
            }
        ...
        if (!seconds || seconds > maxDuration) {
          ...
          return sendErrorReply(
            `The sent video is longer than ${maxDuration} seconds! Send a shorter video.`
          );
        }
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          console.error(
            `Attempt ${attempt} to send sticker failed:`,
            stickerError.message
          );

          if (attempt === 3) {
            throw new Error(
              `Failed to send sticker after 3 attempts: ${stickerError.message}`
            );
          }
      ...
      console.error("Detailed error in sticker command:", error);
      ...
      if (
        ...
        throw new Error(
          `Connection error downloading multimedia from WhatsApp. Try again in a few seconds.`
        );
      }

      if (error.message.includes("FFmpeg")) {
        throw new Error(
          `Error processing multimedia with FFmpeg. Check if the file is not corrupted.`
        );
      }

      throw new Error(`Error processing sticker: ${error.message}`);
    }
  },
};
